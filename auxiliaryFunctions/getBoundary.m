...
xBoundary = [linspace(xBounds(1),xBounds(2),round(np*diff(xBounds))),...
    linspace(xBounds(2),xBounds(2),round(np*diff(yBounds))),...
    linspace(xBounds(2),xBounds(1),round(np*diff(xBounds))),...
    linspace(xBounds(1),xBounds(1),round(np*diff(yBounds)))];

yBoundary = [linspace(yBounds(1),yBounds(1),round(np*diff(xBounds))),...
    linspace(yBounds(1),yBounds(2),round(np*diff(yBounds))),...
    linspace(yBounds(2),yBounds(2),round(np*diff(xBounds))),...
    linspace(yBounds(2),yBounds(1),round(np*diff(yBounds)))];

heading = [zeros(1,round(np*diff(xBounds))),...
    pi/2*ones(   1,round(np*diff(yBounds))),...
    pi*ones(     1,round(np*diff(xBounds))),...
    3*pi/2*ones( 1,round(np*diff(yBounds)))];

bounds = cumsum([0,round(np*diff(xBounds)),round(np*diff(yBounds)),...
    round(np*diff(xBounds)),round(np*diff(yBounds))]);
